...
const playMusic = (track, pause = false) => {
    ...
    if (!pause) {
        ...
        document.querySelector(".circle").classList.remove("pulse")
    }
...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            // Get the last part of the URL path as the folder name
            let url = new URL(e.href);
            let parts = url.pathname.split("/").filter(p => p.length > 0);
            if (parts.length < 2 || parts[0] !== "songs") continue;
            let folder = parts[1];
            
            // Get the metadata of the folder
            try {
                let a = await fetch(`/songs/${folder}/info.json`);
                if (!a.ok) continue; // Skip if info.json doesn't exist
                let response = await a.json(); 
                cardContainer.innerHTML = cardContainer.innerHTML + ` <div data-folder="${folder}" class="card">
                <div class="play">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M5 20V4L19 12L5 20Z" stroke="#ffffff" fill="#ffffff" stroke-width="1.5"
